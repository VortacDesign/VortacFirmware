...
class AngleCalibration:
    def __init__(self, config):
        ...
        self.sample_steps = config.getint('sample_steps', 1)
        self.sample_speed = config.getint('sample_speed', 40)
    ...
    def do_calibration_moves(self, gcmd):
        ...
        microsteps, full_steps = self.get_microsteps()
        meas_step_res = self.sample_steps
        meas_steps = full_steps // meas_step_res
        mcu_stepper = self.mcu_stepper
        step_dist = mcu_stepper.get_step_dist()
        full_step_dist = step_dist * microsteps
        meas_step_dist = full_step_dist * meas_step_res
        rotation_dist = full_steps * full_step_dist
        align_dist = step_dist * self.get_stepper_phase()
        move_speed = self.sample_speed
        gcmd.respond_info("started Angle Calibration with {} resolution resulting in {} steps with stepdistance of {}".format(meas_step_res, meas_steps, meas_step_dist))
        move(mcu_stepper, -(rotation_dist+align_dist), move_speed)
        move(mcu_stepper, 2. * rotation_dist, move_speed)
        move(mcu_stepper, -2. * rotation_dist, move_speed)
        move(mcu_stepper, .5 * rotation_dist - meas_step_dist, move_speed)
        # Move to each full step position
        toolhead = self.printer.lookup_object('toolhead')
        times = []
        samp_dist = meas_step_dist
        for i in range(2 * meas_steps): #internal error here
            ...
            if i == meas_steps-1:
                ...
                move(mcu_stepper, .5 * rotation_dist, move_speed) #compensate gearing slack
                move(mcu_stepper, -.5 * rotation_dist + samp_dist, move_speed) #compensate gearing slack
                samp_dist = -samp_dist
        move(mcu_stepper, .5*rotation_dist + align_dist, move_speed)
        gcmd.respond_info(
            "mesurements finished total number of messages %d" % (len(msgs)))
        toolhead.wait_moves()
        ...
        gcmd.respond_info("finished fetching and sorting data")
        fcal = { i: cal[i] for i in range(meas_steps) }
        rcal = { meas_steps-i-1: cal[i+meas_steps] for i in range(meas_steps) }
        return fcal, rcal
    ...
    def cmd_ANGLE_CALIBRATE(self, gcmd):
        ...
        gcmd.respond_info("finished calibration moves with %d forwad points and %d reverse points"
                          % (len(fcal), len(rcal)))
        microsteps, full_steps = self.get_microsteps()
        meas_step_res = self.sample_steps
        meas_steps = full_steps // meas_step_res
        ...
        merged = { i: fcal[i] + rcal[i] for i in range(meas_steps) }
        ...
        anglist = [angles[i] % 0xffff for i in range(meas_steps)]
...
class Angle:
    def __init__(self, config):
        ...
        self.batch_bulk.add_mux_endpoint("angle/dump_angle",
                                         "sensor", self.name, api_resp) # API endpoint to read mesurements
    def _build_config(self):
